...
get_newswhip_articles <- function(api_key, limit, start_time, end_time) {
  api_endpoint <- paste0('https://api.newswhip.com/v1/articles?key=', api_key)          
  data <- paste0('{\"filters\": [\"language: en"],
                           \"size\": ', limit, ', 
                           \"from\": ', start_time, ',
                           \"to\": ', end_time, ',
...
#### variables to change 
#### YYYY-MM-DD format
days<-as.character(as.Date(as.Date("2020-01-01"):as.Date("2021-12-31"), origin="1970-01-01"))
...
########################################################
######### COVID Keyword Search in Headlines ##########
#######################################################

####load covid keyword list######
load("XXXXXXXX/masterkeywords.Rda")

###make a temporary folder and remove main one for space
temp<-data5 

###lower headlines
temp$headline<-tolower(temp$headline)

###create a loop
mylist <- list()
for (i in masterkeywords$keywords) {
  keyworded <- temp %>%filter(str_detect(headline, i)) %>% mutate(keywordcovid = i)
  mylist[[i]] <- keyworded}
df <- do.call("rbind",mylist)%>%data.frame()

dftemp<-df %>% group_by(uuid, headline) %>% 
  summarise(keywordcovid = paste(keywordcovid,collapse=','))

datacovid<-left_join(dftemp, temp, by="uuid")
datacovid$has_covid_term_headline <- "TRUE"
datacovid$pinkslime<-"empty"
datacovid$newsguard_covid<-"empty"

###rename columns
colnames(datacovid)[colnames(datacovid) == 'headline.x'] <- 'headline'  

##rearrange the data so all column order are the same
datacovid<- datacovid %>% select(delta_time, recent_fb_counts, 
                                 recent_tw_counts, 
                                 predicted_interactions, 
                                 predicted_timestamp, 
                                 uuid, 
                                 ...
                                 max_nw_score, 
                                 fb_data.total_engagement_count, 
                                 fb_data.likes, fb_data.comments, 
                                 fb_data.shares, 
                                 fb_data.total_count_delta, 
                                 fb_data.delta_period, 
                                 fb_data.delta_period_unit, 
                                 fb_data.fb_overperforming, 
                                 tw_data.tw_count, 
                                 tw_data.total_count_delta, 
                                 tw_data.delta_period, 
                                 tw_data.delta_period_unit, 
                                 tw_data.tw_overperforming, 
                                 li_data.li_count, 
                                 li_data.total_count_delta, 
                                 li_data.delta_period, 
                                 li_data.delta_period_unit, 
                                 ...
                                 date_time, 
                                 date, 
                                 keywordcovid, has_covid_term_headline)

total_num_articles<-nrow(datacovid)
daterange <- ' 2020_2021'

###save them as csv and Rda
write.csv(datacovid, file = paste("XXX/Covidstories_EN_starting_", gsub("-", "_", daterange), "_N_", total_num_articles, ".csv", sep=""), row.names=F)
save(datacovid,file=paste("XXXX/Covidstories_EN_starting_", gsub("-", "_", starting_date), "_N_", total_num_articles, ".Rda", sep=""), row.names=F)
